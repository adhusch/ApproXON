...
% This function smoothpatch smooths a triangulated Mesh, with accurate
...
if(mode==1)
    ...
    Ne=ea_vertex_neighbours(FV);
    % Do the curvature weighted smoothing
    [Vx,Vy,Vz]=ea_smoothpatch_curvature_double(double(FV.faces(:,1)),double(FV.faces(:,2)),double(FV.faces(:,3)),double(FV.vertices(:,1)),double(FV.vertices(:,2)),double(FV.vertices(:,3)),ceil(double(itt)),[double(lambda) double(sigma)],Ne);
else
    % Do the umbrella inverse distance weighted smoothing
    [Vx,Vy,Vz]=ea_smoothpatch_inversedistance_double(double(FV.faces(:,1)),double(FV.faces(:,2)),double(FV.faces(:,3)),double(FV.vertices(:,1)),double(FV.vertices(:,2)),double(FV.vertices(:,3)),ceil(double(itt)),[double(lambda) double(sigma)]);
end
